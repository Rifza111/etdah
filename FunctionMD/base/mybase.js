 ...
 if (m.message) { 
  ...
  m.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
        let quoted = message.msg ? message.msg : message
        let mime = (message.msg || message).mimetype || ''
        let messageType = message.m.quotedMsg.type ? message.m.quotedMsg.type.replace(/Message/gi, '') : mime.split('/')[0]
 ...
 var templates = await generateWAMessageFromContent(id, {
  ...
  m.sendDocument = async (jid, source, name, quoted, options) => {
